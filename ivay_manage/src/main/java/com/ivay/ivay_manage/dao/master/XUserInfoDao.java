...
public interface XUserInfoDao {
    ...
    @Insert("insert into x_user_info(phone,password,user_gid,create_time,user_status,account_status, enable_flag)" +
            " values(#{phone},#{password},#{userGid},#{createTime}, #{userStatus}, #{accountStatus}, #{enableFlag})")
    ...
    @Update("update x_user_info set canborrow_amount=canborrow_amount+#{canborrowAmount} where user_gid=#{userGid}")
    ...
    @Select("select count(1) from x_user_contacts where user_gid=#{userGid}")
