...
public interface XRecordLoanDao {
    ...
    @Select("select * from x_record_loan t where t.gid = #{gid}")
    XRecordLoan getByLoanGid(@Param("gid") String loanGid);
    ...
    @Select("SELECT r.order_id, r.user_gid, u.`name`,r.overdue_fee,r.overdue_interest,r.due_amount FROM x_record_loan r "
            + "LEFT JOIN x_user_info u ON r.user_gid = u.user_gid WHERE r.loan_status = 1 AND r.repayment_status != 2 " +
            "and r.order_id NOT in(SELECT order_id from x_virtual_account x where x.response_code=200)")
    ...
    // 查询逾期的用户-逾期提醒
    @Select("SELECT r.user_gid,r.overdue_fee,r.overdue_interest,r.gid,r.due_time,r.due_amount,u.fmc_token," +
            " DATEDIFF(SYSDATE(),r.due_time)due_date FROM x_record_loan r LEFT JOIN x_user_info u ON r.user_gid = u.user_gid" +
            " WHERE( r.due_time <= date_format(sysdate(), '%Y%m%d'))AND u.id IS NOT NULL AND r.repayment_status != 2 AND r.loan_status = 1 ")
     ...
     * @return
     */
    @Select("SELECT x_record_loan.gid loan_gid, x_baokim_transfers_info.reference_id FROM x_record_loan " +
